...
export function WritingSidebar({ posts }: { posts: EditorialPost[] }) {
  ...
  const counts: Record<string, number> = { essay: 0, sketchnote: 0, retro: 0, til: 0 };
  posts.forEach((p) => { counts[inferKind(p.tags)]++; });
  const kinds = [
    { k: 'all', label: 'Everything', count: total, on: true },
    { k: 'essay', label: 'Essays', count: counts.essay, on: false },
    { k: 'sketchnote', label: 'Sketchnotes', count: counts.sketchnote, on: false },
      ...
      <SidebarCard title="Filter by kind">
        <div className="flex flex-col gap-1.5">
          {kinds.map((it) => (
            ...
            >
              <span className={`text-[13px] text-foreground ${it.on ? 'font-bold' : 'font-medium'}`}>
                {it.label}
              </span>
