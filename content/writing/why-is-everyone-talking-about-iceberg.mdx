...
- **Zero ACID & brittle schema.** Adding a column meant _drop + recreate_—and pray downstream jobs didn't explode.
...
3. **Catalog** – pointer to the _current_ snapshot.
...
New data files _and_ a brand-new snapshot are created for every new write. Metadata keeps track of which snapshot to choose. So there is a point-in-time consistent view of the data.

<figure>
  <img
    src="/blog_images/timestamp_snapshot_iceberg.gif"
    alt="Snapshot Write Iceberg"
  />
  <figcaption>
    Source:{' '}
    <a
      href="https://netflixtechblog.com/incremental-processing-using-netflix-maestro-and-apache-iceberg-b8ba072ddeeb"
      target="_blank"
...
> **My takeaway:** No global locks; writers only fail when they touch the _same_ rows. Spark, Flink, and Trino all leverage this commit model out of the box.
...
Another useful benefit of this is snapshots + retention policies = built-in audit _and_ GDPR compliance (`expire_snapshots` after 90 days).
...
- Resist the urge to bucket by _every_ dimension—Iceberg stats already prune aggressively.
