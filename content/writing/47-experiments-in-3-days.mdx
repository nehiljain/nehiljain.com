...
description: "A customer's batch embeddings pipeline was crashing at 10M rows. 47 autonomous scored experiments later, it shipped 3.2× faster on 60% fewer GPUs."
date: 2026-05-12
...
The first ~10 scored experiments went well — the early 20 log entries were mostly crashes; the first scored run was entry #32. Claude Code would read the log, pick the next hypothesis, edit the pipeline code or job config, submit it through `/run` (another Anyscale Claude skill), watch metrics roll in via `/inspect`, score, append. A senior engineer might have gotten here in maybe 3 experiments instead of 10. But each one would have taken much longer.
...
Around scored experiment #10 (log entry ~50), the loop started stuttering. The agent submitted a config it had already tried — same shuffle strategy, same partition count, different variable name. Then a slight variation of something it had rejected at scored experiment 4. Then it circled back to scored experiment 7's approach with different names. Three cycles going nowhere. The `experiment.log` tracked all the outcomes but not the reasoning. The agent knew _what happened_. It didn't know _what to try next_ or _why something had been ruled out_.
...
By log entry 75, the gains were flattening. The agent was tweaking CPU counts. ~47 scored experiments in the campaign (entry #83 in the full log), the pipeline ran 10M rows reliably, on half the GPUs, in under three days.

![Composite score over experiments — the autoresearch loop converging.](/blog_images/ralph-loop-autotune-progress.webp)
...
That was the shift. Optimization wasn't something I _did_ anymore — it was something I _set up_ and walked away from. The work that actually took thinking was upfront, in `scoring.py` and `objective.md`. Verification-driven optimization, basically. TDD for infra. Get the scoring function wrong and the agent will optimize against the wrong thing for 47 experiments straight and call it a win.

I think this is where platform engineering is heading. Not writing the optimization. Writing the loop that finds it. The objective file, the experiment log, the scoring function — that's the new deliverable. The agent runs the search. You choose the search space. And choosing the search space well is the whole skill now.
